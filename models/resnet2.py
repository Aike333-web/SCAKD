...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_filters, block_name='BasicBlock', num_classes=10):
        ...
        self.auxiliary1 = nn.Sequential(
            SepConv(
                channel_in=64 * block.expansion,        # 64
                channel_out=128 * block.expansion       # 128
            ),
            SepConv(
                channel_in=128 * block.expansion,       # 128
                channel_out=256 * block.expansion       # 256
            ),
            SepConv(
                channel_in=256 * block.expansion,       # 256
                channel_out=512 * block.expansion       # 512
            ),
            nn.AvgPool2d(4, 4),
        )
        ...
        # self.auxiliary1 = nn.Sequential(
        #     SepConv(
        #         channel_in=16 * block.expansion,  # 64
        #         channel_out=32 * block.expansion  # 128
        #     ),
        #     SepConv(
        #         channel_in=32 * block.expansion,  # 128
        #         channel_out=64 * block.expansion  # 256
        #     ),
        #     SepConv(
        #         channel_in=64 * block.expansion,  # 256
        #         channel_out=128 * block.expansion  # 512
        #     ),
        #     nn.AvgPool2d(4, 4)
        # )
        #
        ...
        self.fc_aux = nn.Linear(512, 100)
        # self.fc_aux = nn.Linear(128, 100)
    ...
    def forward(self, x, is_feat=False, preact=False):
        ...
        out1_feature = self.fc_aux(self.auxiliary1(feature_list[0]).view(x.size(0), -1))
        out2_feature = self.fc_aux(self.auxiliary2(feature_list[1]).view(x.size(0), -1))
        out3_feature = self.fc_aux(self.auxiliary3(feature_list[2]).view(x.size(0), -1))
        ...
        if is_feat:
            if preact:
                ...
                return x, [f0, f1, f2, f3, f4], feat_list  # 最后输出, 中间层的4维特征, 辅助分类器得到的二维特征
        else:
            return x
