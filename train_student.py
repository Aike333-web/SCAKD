...
from distiller_zoo import PKT, DistillKL, DKDloss, Similarity, VIDLoss, SCAKDloss,FeatureLoss
...
def parse_option():
    ...
    parser.add_argument('--model_s', type=str, default='resnet8',
                        choices=['resnet8', 'resnet14', 'resnet20', 'resnet32', 'resnet44', 'resnet56', 'resnet110',
                                 'ResNet18', 'ResNet34', 'resnet8x4', 'resnet32x4', 'wrn_16_1', 'wrn_16_2', 'wrn_40_1', 'wrn_40_2', 'wrn_50_2',
                                 'vgg8', 'vgg11', 'vgg13', 'vgg16', 'vgg19', 'ResNet50',
                                 'MobileNetV2', 'ShuffleV1', 'ShuffleV2'])
    parser.add_argument('--path-t', type=str, default=None, help='teacher model snapshot')

    # distillation
    parser.add_argument('--distill', type=str, default='kd', choices=['kd', 'similarity', 'vid', 'pkt', 'crd', 'dkd', 'srrl', 'scakd'])
    ...
    parser.add_argument('-e', '--weight_e', type=float, default=1.0, help='weight balance for semckd')
    parser.add_argument('-f', '--weight_SSL', type=float, default=1.0, help='weight balance for SSL')
    # KL distillation
...
def load_teacher(model_path, n_cls, gpu=None, opt=None):
    ...
    if opt.dataset == 'cifar100':
        if(model_t=='ResNet50'):
            model.load_state_dict(state_dict=torch.load(model_path)['state_dict'])
        else:
            model.load_state_dict(torch.load(model_path, map_location=map_location)['model'],False)
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    elif opt.distill == 'scakd':
        criterion_kd = SCAKDloss()
        s_n = [f.shape[1] for f in feat_s[1:-1]]
        t_n = [f.shape[1] for f in feat_t[1:-1]]
        criterion_kd1 = FeatureLoss()
        self_attention = SelfA(len(feat_s) - 2, len(feat_t) - 2, opt.batch_size*3, s_n, t_n)
    ...
    if opt.distill in ['scakd']:
        criterion_list.append(criterion_kd1)
    ...
    if opt.dataset == 'cifar100':
        ...
        train_loader, val_loader, train_sampler = get_imagenet_dataloader(dataset=opt.dataset, batch_size=opt.batch_size, num_workers=opt.num_workers, multiprocessing_distributed=opt.multiprocessing_distributed)
    else:
        raise NotImplementedError(opt.dataset)
    best_acc = 0
